...
class IsicDashboard(models.AbstractModel):
    _name = "isic.dashboard"
    _description = "Dashboard ISIC"
    ...
    def retrieve_dashboard(self):
        ...
        # Read active sections, ordered by sequence
        all_sections = self.env["isic.dashboard.section"].search([])

        # Filter: sections where user belongs to at least one allowed group,
        # or sections with no group restriction (visible to all).
        visible_sections = all_sections.filtered(lambda s: not s.group_ids or (user.group_ids & s.group_ids))

        data = {
            "user_name": user.name,
            "annee_academique": annee.name if annee else "",
            "sections": [],
            "charts": [],
        }

        for section in visible_sections:
            # Convention: _section_{code}(annee) → list of section dicts
            method = getattr(self, f"_section_{section.code}", None)
            if method:
                data["sections"].extend(method(annee))

            # Convention: _chart_{code}(annee) → list of chart dicts
            if section.has_chart:
                chart_method = getattr(self, f"_chart_{section.code}", None)
                if chart_method:
                    data["charts"].extend(chart_method(annee))
    ...
    @api.model
    def _section_approbation(self, annee):
    ...
    @api.model
    def _chart_ged(self, annee):
        """Bar chart: documents GED created per month (last 6 months)."""
    ...
    @api.model
    def _chart_approbation(self, annee):
        """Doughnut + line chart for approval requests."""
        charts = [self._chart_demandes_by_state()]
        user = self.env.user
