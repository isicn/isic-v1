...
class TestDmsField(BaseCommon):
    ...
    def setUpClass(cls):
        ...
        cls.storage = cls.env["dms.storage"].create(
            {"name": "Test Storage (dms_field)", "save_type": "database"}
        )
        cls.access_group = cls.env["dms.access.group"].create(
            {
                "name": "Test Access Group (dms_field)",
                "perm_create": True,
                "perm_write": True,
                "perm_unlink": True,
        ...
        # _with_parent tests to use as parent_directory_id
        cls.storage_root_directory = cls.env["dms.directory"].create(
            {
                "name": "Storage Root",
