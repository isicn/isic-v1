...
class DmsDirectory(models.Model):
    ...
    def _search_starred(self, operator, operand):
        # Odoo 19: "=" is normalized to "in" before calling search methods,
        # so operand may be a collection (e.g., OrderedSet({True}))
        if isinstance(operand, (list, tuple, set, frozenset)) or (
            hasattr(operand, "__iter__") and not isinstance(operand, str)
        ):
            value = any(operand)
        else:
            value = bool(operand)
        positive = (operator not in NEGATIVE_TERM_OPERATORS) == value
        if positive:
