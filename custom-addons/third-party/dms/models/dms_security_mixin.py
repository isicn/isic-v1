...
class DmsSecurityMixin(models.AbstractModel):
    ...
    def _get_permission_domain(self, operator, value, operation):
        ...
        _self = self
        # In Odoo 19, security domain optimization always runs in sudo mode
        # (optimize_full(self_sudo)). The original hack checked value == uid,
        # but Odoo 19's boolean normalization converts the user ID to True
        # before calling search methods, making that check impossible.
        # Instead, we always drop sudo to compute real DMS permissions.
        if self.env.su:
