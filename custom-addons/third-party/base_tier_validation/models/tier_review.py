...
class TierReview(models.Model):
    ...
    def _compute_can_review(self):
        # Odoo 19: the status side-effect below may or may not persist
        # depending on whether the ORM triggers this compute vs explicit call.
        # _can_review_value is adapted to work correctly either way.
    ...
    def _advance_pending_status(self):
        """Advance waiting reviews to pending based on approve_sequence.

        In Odoo 19, side-effect writes to non-computed fields inside stored
        compute methods may not persist. This method is called explicitly
        after validation/rejection to ensure correct status transitions.
        """
        active = self.filtered(lambda r: r.status in ("waiting", "pending"))
        if not active:
            return
        next_seq = min(active.mapped("sequence"))
    ...
    def _can_review_value(self):
        ...
        # Odoo 19: consider both waiting and pending reviews for sequence
        # determination (status side-effect may not have persisted)
        active_reviews = resource.review_ids.filtered(
            lambda r: r.status in ("waiting", "pending")
        )
        if not active_reviews:
            return True
        sequence = min(active_reviews.mapped("sequence"))
