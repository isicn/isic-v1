...
class TierValidation(models.AbstractModel):
    ...
    def _search_reviewer_ids(self, operator, value):
        model_operator = "in"
        # Odoo 19: the ORM normalizes ("reviewer_ids", "=", False) into
        # ("reviewer_ids", "in", OrderedSet([False])) for Many2many fields.
        # Detect both the original form and the normalized form.
        is_false_search = False
        if operator == "=" and (not value or value == "[]"):
            is_false_search = True
        elif operator == "in" and hasattr(value, "__iter__") and set(value) == {False}:
            is_false_search = True
        if is_false_search:
            # Search for records that have not yet been through a validation
            # process: find all reviewed records and return the complement.
        ...
        if self._tier_validation_state_field_is_computed and isinstance(self.id, int):
            self.env.cr.execute(
                SQL(
                    ...
                    SQL.identifier(self._state_field),
    ...
    def _update_counter(self, review_counter):
        # Odoo 19: advance pending status explicitly, can_review recomputes
        # automatically via @api.depends("status")
        self.review_ids._advance_pending_status()
