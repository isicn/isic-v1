...
class ResConfigSettings(models.TransientModel):
    ...
    theme_background_image_light = fields.Binary(related="company_id.background_image_light", readonly=False)

    theme_background_image_dark = fields.Binary(related="company_id.background_image_dark", readonly=False)

    theme_color_appbar_text_light = fields.Char(string="AppsBar Text Light Color")

    theme_color_appbar_active_light = fields.Char(string="AppsBar Active Light Color")

    theme_color_appbar_background_light = fields.Char(string="AppsBar Background Light Color")

    theme_color_appbar_text_dark = fields.Char(string="AppsBar Text Dark Color")

    theme_color_appbar_active_dark = fields.Char(string="AppsBar Active Dark Color")

    theme_color_appbar_background_dark = fields.Char(string="AppsBar Background Dark Color")
    ...
    def _get_light_theme_color_values(self):
        return self.env["muk_web_colors.color_assets_editor"].get_color_variables_values(
            self.COLOR_ASSET_THEME_LIGHT_URL, self.COLOR_BUNDLE_THEME_LIGHT_NAME, self.THEME_COLOR_FIELDS
        )

    def _get_dark_theme_color_values(self):
        return self.env["muk_web_colors.color_assets_editor"].get_color_variables_values(
            self.COLOR_ASSET_THEME_DARK_URL, self.COLOR_BUNDLE_THEME_DARK_NAME, self.THEME_COLOR_FIELDS
        )
    ...
    def _replace_light_theme_color_values(self):
        variables = [{"name": field, "value": self[f"theme_{field}_light"]} for field in self.THEME_COLOR_FIELDS]
        return self.env["muk_web_colors.color_assets_editor"].replace_color_variables_values(
            self.COLOR_ASSET_THEME_LIGHT_URL, self.COLOR_BUNDLE_THEME_LIGHT_NAME, variables
        )

    def _replace_dark_theme_color_values(self):
        variables = [{"name": field, "value": self[f"theme_{field}_dark"]} for field in self.THEME_COLOR_FIELDS]
        return self.env["muk_web_colors.color_assets_editor"].replace_color_variables_values(
            self.COLOR_ASSET_THEME_DARK_URL, self.COLOR_BUNDLE_THEME_DARK_NAME, variables
        )
