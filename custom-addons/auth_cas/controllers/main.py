...
class CASAuthController(http.Controller):
    ...
    def cas_callback(self, **kw):
        ...
        try:
            ...
            login = request.env["res.users"].sudo()._auth_oauth_signin(provider.id, user_info, {})
    ...
    def _validate_cas_ticket(self, provider, ticket):
        ...
        try:
            ...
            if response.status_code != 200:
                _logger.error("CAS validation failed with status %s: %s", response.status_code, response.text[:500])
                return None
    ...
    def _parse_cas_json_response(self, data):
        ...
        if "authenticationFailure" in service_response:
            ...
            _logger.error("CAS authentication failure: %s - %s", failure.get("code"), failure.get("description"))
            return None
...
class CASLogoutController(_LogoutBase):
    ...
    @http.route(
        "/web/session/logout", type="http", auth="none", website=True, multilang=False, sitemap=False, readonly=True
    )
    def logout(self, redirect="/odoo"):
        ...
        if uid:
            try:
                slo_enabled = request.env["ir.config_parameter"].sudo().get_param("auth_cas.slo_enabled", "True")
