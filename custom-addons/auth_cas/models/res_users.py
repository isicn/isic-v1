...
class ResUsers(models.Model):
    ...
    cas_uid = fields.Char(string="CAS UID", help="Identifiant unique de l'utilisateur dans le CAS")

    cas_last_sync = fields.Datetime(string="Last CAS Sync", help="Dernière synchronisation des attributs CAS")

    cas_attributes = fields.Text(string="CAS Attributes (JSON)", help="Derniers attributs reçus du CAS (pour debug)")
    ...
    def _cas_signin(self, provider, validation, params):
        ...
        cas_uid = (
            validation.get("user")
            or validation.get("user_id")
            or validation.get("uid")
            or validation.get("username")
            or validation.get(attr_map.get("login", "uid"))
            or validation.get("id")
        )

        email_raw = (
            validation.get("mail") or validation.get("email") or validation.get(attr_map.get("email", "mail"), "")
        )
        ...
        name_raw = (
            validation.get("cn")
            or validation.get("displayName")
            or validation.get("name")
            or validation.get(attr_map.get("name", "cn"), cas_uid)
        )
        ...
        if user:
            ...
            return self._cas_create_user(provider, validation, cas_uid, email, name)
    ...
    def _cas_create_user(self, provider, validation, cas_uid, email, name):
        ...
        _logger.info("Creating CAS user: login=%s, name=%s, email=%s, is_internal=%s", login, name, email, is_internal)
        ...
        try:
            ...
            if is_internal and group_ids:
                ...
                _logger.info("CAS internal user created: %s (id=%d) with groups %s", login, new_user.id, group_ids)
            else:
    ...
    def _cas_sync_partner_fields(self, validation):
        ...
        Champs synchronisés :
        - cn / givenName + sn → name (nom complet)
        - mail → email
        ...
        # Synchroniser le nom depuis LDAP cn / givenName + sn
        cn = self._cas_extract_attr(validation, "cn") or self._cas_extract_attr(validation, "displayName")
        given_name = self._cas_extract_attr(validation, "givenName")
        sn = self._cas_extract_attr(validation, "sn")
        if given_name and sn:
            partner_vals["name"] = f"{given_name} {sn}"
        elif cn:
            partner_vals["name"] = cn
    ...
    def _cas_resolve_groups(self, cas_attributes, provider_id=None):
        ...
        """
        return self.env["auth.cas.group.mapping"].resolve_groups_from_cas(cas_attributes, provider_id)
