...
class AuthOAuthProvider(models.Model):
    ...
    is_cas_provider = fields.Boolean(
        string='CAS Provider',
        default=False,
        help="Cocher si ce provider est un serveur CAS utilisant OAuth2"
    )

    cas_server_url = fields.Char(
        string='CAS Server URL',
        help="URL de base du serveur CAS (ex: https://cas.isic.ma/cas)"
    )
    ...
    client_secret = fields.Char(
        ...
        help="Secret OAuth2 fourni par le serveur CAS"
    )
...
Exemple: {"login": "uid", "email": "mail", "name": "cn", "groups": "memberOf"}"""
    ...
    cas_use_pkce = fields.Boolean(
        string='Use PKCE',
        default=False,
        help="Utiliser PKCE (Proof Key for Code Exchange) pour plus de sécurité"
    )

    cas_scope = fields.Char(
        string='OAuth Scopes',
        default='openid profile email',
        help="Scopes OAuth2 à demander au serveur CAS"
    )
