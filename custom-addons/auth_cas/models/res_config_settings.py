...
class ResConfigSettings(models.TransientModel):
    ...
    cas_enabled = fields.Boolean(
        string='Enable CAS SSO',
        config_parameter='auth_cas.enabled',
        help="Activer l'authentification CAS SSO"
    )
    ...
    cas_client_secret = fields.Char(
        string='CAS Client Secret',
        config_parameter='auth_cas.client_secret',
        help="Client Secret OAuth2"
    )
    ...
    ldap_enabled = fields.Boolean(
        string='LDAP Enabled',
        compute='_compute_ldap_enabled',
        help="Indique si l'authentification LDAP est configurée"
    )

    ldap_server_count = fields.Integer(
        string='LDAP Servers',
        compute='_compute_ldap_enabled',
        help="Nombre de serveurs LDAP configurés"
    )
    ...
    def _compute_ldap_enabled(self):
        for record in self:
            ldap_servers = self.env['res.company.ldap'].search_count([
